...
impl Plugin for SteamPlugin {
    fn info(&self) -> PluginInfo {
        PluginInfo { id: "steam", version: "0.1.0", description: "Steam 成就 / 统计 / 云文件（脚本插件）" }
    }
    ...
    fn install(&mut self, vm: &mut Vm) {
        ...
        vm.register_native("steam_is_available", move |_ctx, _args| Ok(Value::Bool(rt.borrow().backend.is_available())));

        let rt = Rc::clone(&self.runtime);
        vm.register_native("steam_app_id", move |_ctx, _args| Ok(Value::Number(rt.borrow().backend.app_id() as f64)));
        ...
        vm.register_native("steam_achievement_unlock", move |ctx, args| {
            ...
            rt.borrow_mut().backend.unlock_achievement(&id).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Null)
        });
        ...
        vm.register_native("steam_achievement_unlocked", move |ctx, args| {
            let id = arg_string(ctx, &args, 0)?;
            let ok = rt.borrow().backend.is_achievement_unlocked(&id).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Bool(ok))
        });
        ...
        vm.register_native("steam_achievement_clear", move |ctx, args| {
            let id = arg_string(ctx, &args, 0)?;
            rt.borrow_mut().backend.clear_achievement(&id).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Null)
        });
        ...
        vm.register_native("steam_stat_get", move |ctx, args| {
            ...
            let v = rt.borrow().backend.get_stat(&name).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
        ...
        vm.register_native("steam_stat_set", move |ctx, args| {
            ...
            rt.borrow_mut().backend.set_stat(&name, value).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Null)
        });

        let rt = Rc::clone(&self.runtime);
        vm.register_native("steam_stats_store", move |_ctx, _args| {
            rt.borrow_mut().backend.store_stats().map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Null)
        });
        ...
        vm.register_native("steam_cloud_read", move |ctx, args| {
            ...
            match rt.borrow().backend.cloud_read(&path).map_err(|_| VmError::BadNativeArg { name: "backend" })? {
        ...
        vm.register_native("steam_cloud_write", move |ctx, args| {
            ...
            rt.borrow_mut().backend.cloud_write(&path, &data).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Null)
        });
        ...
        vm.register_native("steam_cloud_delete", move |ctx, args| {
            let path = arg_string(ctx, &args, 0)?;
            let ok = rt.borrow_mut().backend.cloud_delete(&path).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Bool(ok))
        });
        ...
        vm.register_native("steam_overlay_open_url", move |ctx, args| {
            ...
            rt.borrow_mut().backend.overlay_open_url(&url).map_err(|_| VmError::BadNativeArg { name: "backend" })?;
            Ok(Value::Null)
        });
    }
}
