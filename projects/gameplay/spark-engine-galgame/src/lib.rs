...
pub use spark_plugin_live2d::{
    Live2dBackend, Live2dModelId, Live2dPlugin, Live2dRuntime, NullLive2dBackend, LIVE2D_NATIVES,
};
...
impl GalgameEngine {
    ...
    pub fn with_live2d_backend(
        mods_root: impl Into<PathBuf>,
        backend: Box<dyn Live2dBackend>,
    ) -> Self {
        let live2d = Live2dRuntime::new(backend);
        let mut engine = SparkEngine::new(mods_root);
        engine
            .register_plugin(Box::new(Live2dPlugin::new(Rc::clone(&live2d))))
            .expect("默认 Live2D 插件 id 不应冲突");
        Self {
            engine,
            script: ScriptPlayer::default(),
...
mod tests {
    ...
    fn dialogue_and_choice() {
        ...
        gal.script.push_choices(vec![
            Choice {
                label: "去东".into(),
                set_flag: Some(("route".into(), 1.0)),
            },
            Choice {
                label: "去西".into(),
                set_flag: Some(("route".into(), 2.0)),
            },
        ]);
    ...
    fn live2d_plugin_registered_by_default() {
        ...
        let id = gal
            ...
            .borrow_mut()
            .backend
            .load("demo.model3.json")
            .unwrap();
        gal.live2d()
            .borrow_mut()
            .backend
            .set_param(id, "ParamMouthOpenY", 0.8)
            .unwrap();
        let v = gal
            .live2d()
            .borrow()
            .backend
            .get_param(id, "ParamMouthOpenY")
            .unwrap();
