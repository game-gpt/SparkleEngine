...
pub fn discover_and_order(mods_root: &Path) -> Result<Vec<ModManifest>, EngineError> {
    ...
    let rd = std::fs::read_dir(mods_root).map_err(|e| EngineError::from_io(mods_root.display().to_string(), e))?;
    ...
    for m in by_id.values() {
        for dep in &m.dependencies {
            if !by_id.contains_key(dep) {
                return Err(EngineError::MissingDep { mod_id: m.id.clone(), dep: dep.clone() });
            }
        }
    }
...
fn topological_sort(mut by_id: HashMap<String, ModManifest>) -> Result<Vec<ModManifest>, EngineError> {
    ...
    if out.len() != ids.len() {
        ...
        return Err(EngineError::CyclicDeps { mods: left.join(", ") });
    }
    Ok(out)
}
