...
import { detectNativePlatformPackage, listPlatformPackages, resolveNativePath, type SparkPlatformPackage } from "./platform";
...
export function loadSpark(_options: LoadOptions = {}): SparkHostBindings {
    ...
    if (_options.platformPackage === "@game-gpt/sparkle-engine-unknown-wasm32") {
        throw new Error("`loadSpark` 只加载原生 `.node`。Wasm 请使用 `@game-gpt/sparkle-engine-unknown-wasm32` 包自己的入口。");
    }
    const addonPath = resolveNativePath();
    const addon = nodeRequire(addonPath) as SparkAddon;
    if (typeof addon.JsSparkHost !== "function") {
        throw new Error(`原生插件缺少 JsSparkHost：${addonPath}。请重新运行 node scripts/build/napi.mjs`);
    }
