...
use oak_valkyrie::{
    ValkyrieTokenType,
    ast::{
        Block, ExprStmt, Let, MicroDeclaration, Pattern, Statement, StatementNode, StringLiteral, StringSegment, TermExpression, ValkyrieRoot,
    },
};
use spark_ir::{HirBinaryOp, HirExpr, HirFunction, HirModule, HirStmt, HirUnaryOp, HostBindTable, PackageId, Ty};

/// 尝试将整个根降低为 HIR。
pub(crate) fn lower_root_to_hir(root: &ValkyrieRoot, hosts: &HostBindTable) -> Result<HirModule, String> {
    ...
    for item in &root.items {
        match item {
            ...
            StatementNode::Let(l) => {
                body.push(lower_let(l, &mut locals, &mut local_tys, &fn_index, hosts)?);
            }
            StatementNode::ExprStmt(s) => {
                let (stmt, is_ret) = lower_expr_stmt(s, &mut locals, &mut local_tys, &fn_index, hosts)?;
                if is_ret {
                    saw_return = true;
                }
                body.push(stmt);
...
fn lower_micro(m: &MicroDeclaration, fn_index: &HashMap<String, u32>, hosts: &HostBindTable) -> Result<HirFunction, String> {
    let mut locals: HashMap<String, u32> = HashMap::new();
    ...
    let mut local_tys: Vec<(Arc<str>, Ty)> = Vec::new();
    let (body, _) = lower_block_stmts(&m.body, &mut locals, &mut local_tys, fn_index, hosts, true)?;
    Ok(HirFunction { name: Arc::from(m.name.name.as_str()), symbol: None, params, return_ty: Ty::Dynamic, locals: local_tys, body, span: None })
}
...
fn lower_block_stmts(
    ...
    for s in &body.statements {
        match s {
            ...
            Statement::ExprStmt(e) => {
                let (stmt, is_ret) = lower_expr_stmt(e, locals, local_tys, fn_index, hosts)?;
                if is_ret {
                    saw_return = true;
                }
                out.push(stmt);
    ...
    if ensure_return && !saw_return {
        out.push(HirStmt::Return { value: Some(HirExpr::LiteralNull { span: None }), span: None });
        saw_return = true;
    }
    Ok((out, saw_return))
}
...
fn lower_let(
    ...
    Ok(HirStmt::AssignLocal { index, value: lower_expr(&l.expr, locals, fn_index, hosts)?, span: None })
}
...
fn lower_expr_stmt(
    ...
    match &s.expr {
        ...
        TermExpression::If { pattern, condition, then_branch, else_branch, .. } => {
            ...
            let (then_body, _) = lower_block_stmts(then_branch, locals, local_tys, fn_index, hosts, false)?;
            let else_body =
                if let Some(eb) = else_branch { lower_block_stmts(eb, locals, local_tys, fn_index, hosts, false)?.0 } else { Vec::new() };
            Ok((HirStmt::If { cond, then_body, else_body, span: None }, false))
        }
        TermExpression::Loop { condition, pattern, body, .. } => {
            if pattern.is_some() {
                return Err("ir_unsupported_for_loop".into());
            }
            let Some(cond_expr) = condition
            else {
                return Err("ir_unsupported_infinite_loop".into());
            };
            let cond = lower_expr(cond_expr, locals, fn_index, hosts)?;
            let (loop_body, _) = lower_block_stmts(body, locals, local_tys, fn_index, hosts, false)?;
            Ok((HirStmt::While { cond, body: loop_body, span: None }, false))
        }
        _ => Ok((HirStmt::Expr { expr: lower_expr(&s.expr, locals, fn_index, hosts)?, span: None }, false)),
    }
}
...
fn lower_expr(
    ...
    match e {
        TermExpression::Bool { value: true, .. } => Ok(HirExpr::LiteralBool { value: true, span: None }),
        TermExpression::Bool { value: false, .. } => Ok(HirExpr::LiteralBool { value: false, span: None }),
        ...
        TermExpression::NamePath(path) => {
            ...
            if let Some(&idx) = locals.get(name) {
                return Ok(HirExpr::Local { index: idx, span: None });
            }
        ...
        TermExpression::Unary(u) => {
            ...
            Ok(HirExpr::Unary { op, expr, span: None })
        }
        TermExpression::Binary(b) => match b.operator {
            ValkyrieTokenType::AndAnd => Ok(HirExpr::If {
                ...
                else_branch: Box::new(HirExpr::LiteralBool { value: false, span: None }),
                span: None,
            }),
            ValkyrieTokenType::OrOr => Ok(HirExpr::If {
                cond: Box::new(lower_expr(&b.lhs, locals, fn_index, hosts)?),
                then_branch: Box::new(HirExpr::LiteralBool { value: true, span: None }),
        ...
        TermExpression::ApplyCall { callee, args, .. } => lower_call(callee, args, locals, fn_index, hosts),
        TermExpression::Block(body) => {
            // 表达式块：隔离局部，块内 let 不外溢。
            let mut block_locals = locals.clone();
            let mut block_tys = Vec::new();
            let (stmts, _) = lower_block_stmts(body, &mut block_locals, &mut block_tys, fn_index, hosts, false)?;
            Ok(HirExpr::Block { stmts, result: Some(Box::new(HirExpr::LiteralNull { span: None })), span: None })
        }
...
fn lower_literal(lit: &StringLiteral) -> Result<HirExpr, String> {
    ...
    if lit.quote_count == 0 {
        ...
        let n: f64 = text.parse().map_err(|_| format!("ir_invalid_number:{text}"))?;
        return Ok(HirExpr::LiteralNumber { value: n, span: None });
    }
    Ok(HirExpr::LiteralString { value: Arc::from(text), span: None })
}
