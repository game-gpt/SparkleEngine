...
`@game-gpt/sparkle-engine` 的 linux/x64 原生平台袋：预编译 N-API 插件（`.node`），无 TypeScript 入口。

## 包内容

| 字段         | 值                                   |
|--------------|--------------------------------------|
| 包名         | `@game-gpt/sparkle-engine-linux-x64` |
...
产物写入 `projects/platforms/native/sparkle-engine-linux-x64/`。跨平台发布需在对应机器或 CI 上分别构建。
