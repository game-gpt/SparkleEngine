...
impl PingPongGame {
    ...
    fn bounce_paddle(ball: &mut Ball, paddle: &Paddle) {
        ...
        let cy = ball.y;
        let r = ball.radius;
        let hit = cx + r >= paddle.x
            && cx - r <= paddle.x + paddle.w
            && cy + r >= paddle.y
            && cy - r <= paddle.y + paddle.h;
        if !hit {
            return;
        }
...
impl GameHost for PingPongGame {
    ...
    fn draw(&mut self, draw: &mut DrawList) {
        draw.begin_world();
        draw.fill_rect(
            Rect::new(0.0, 0.0, COURT_W, COURT_H),
            Color::rgb(0.05, 0.07, 0.10),
        );
        let mut y = 8.0_f32;
        while y < COURT_H {
            draw.fill_rect(
                Rect::new(COURT_W * 0.5 - 2.0, y, 4.0, 12.0),
        ...
        draw.fill_rect(
            Rect::new(self.left.x, self.left.y, self.left.w, self.left.h),
            paddle_c,
        );
        draw.fill_rect(
            Rect::new(self.right.x, self.right.y, self.right.w, self.right.h),
            paddle_c,
        );
