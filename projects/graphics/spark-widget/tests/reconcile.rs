...
use spark_widget::{
    UiRuntime, WidgetTree, button_widget, column, label_widget,
};
...
fn reconcile_reuses_ids_by_key() {
    ...
    assert_eq!(tree.node(second).unwrap().children, vec![
        b0,
        a0,
        tree.child_by_key(second, "c").unwrap(),
    ]);
}
...
fn reconcile_drops_removed_keys() {
    ...
    let menu = column()
        .key("menu")
        .child(button_widget().key("a"))
        .child(button_widget().key("gone"))
        .mount(&mut tree, root)
        .unwrap();
...
fn reconcile_scene_preserves_hover_key() {
    ...
    let id = runtime
        .mount_scene(
            column()
                .key("title-menu")
                .child(button_widget().key("title-0").text("单人"))
                .child(button_widget().key("title-1").text("多人")),
        )
        .unwrap();
    ...
    assert_eq!(
        runtime.tree.node(btn).unwrap().content.text.as_deref(),
        Some("多人模式")
    );
}
...
fn reconcile_scene_remounts_when_root_key_changes() {
    let mut runtime = UiRuntime::new();
    let id = runtime
        .mount_scene(column().key("title-menu").child(label_widget().text("a")))
        .unwrap();
    let id2 = runtime
        .reconcile_scene(column().key("pause").child(label_widget().text("b")))
        .unwrap();
    assert_ne!(id2, id);
