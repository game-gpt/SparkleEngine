...
mod tests {
    ...
    fn hover_enter_and_leave() {
        ...
        let viewport = Rect::new(0.0, 0.0, 320.0, 240.0);
        {
            ...
            let response = ui.button("悬停");
            assert!(response.hover_entered);
            assert!(!response.hover_left);
            ui.end();
        }
        {
            let mut input = Input::default();
    ...
    #[test]
    fn password_field_masks_drawn_text() {
        let mut state = UiState::new();
        let mut draw = DrawList::new(Color::rgb(0.0, 0.0, 0.0));
        let viewport = Rect::new(0.0, 0.0, 320.0, 240.0);
        ...
        let id = ui.password_field(&mut value).id;
        ui.request_focus(id);
        ui.end();
