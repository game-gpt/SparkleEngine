...
pub fn tab_view() -> WidgetBuilder {
    WidgetBuilder::new(WidgetKind::TabView).layout(LayoutSpec {
        width: Size::Fill,
        height: Size::Fill,
        ..LayoutSpec::vertical()
    })
...
pub fn sync_tabs(
    tree: &mut WidgetTree,
    tab_view_id: WidgetId,
    selected: usize,
    tabs: &[(&str, WidgetBuilder)],
) -> Option<WidgetId> {
    ...
    let mut bar = row().key("tab-bar").layout(LayoutSpec {
        width: Size::Fill,
        height: Size::Px(36.0),
        gap: 4.0,
        ...
        bar = bar.child(
            button_widget()
                ...
                .style(crate::style::Style {
                    opacity: Some(if selected_tab { 1.0 } else { 0.75 }),
    ...
    let mut pages = column().key("tab-pages").layout(LayoutSpec {
        width: Size::Fill,
        height: Size::Fill,
        ..LayoutSpec::vertical()
    });
...
pub fn handle_tab_click(tree: &mut WidgetTree, clicked: WidgetId) -> Option<usize> {
    ...
    let page_count = tree
        .node(tab_view)
        .and_then(|n| n.children.get(1).copied())
        .and_then(|pages| tree.node(pages).map(|p| p.children.len()))
        .unwrap_or(0);
...
mod tests {
    ...
    fn sync_tabs_shows_only_selected_page() {
        ...
        sync_tabs(
            ...
            1,
            &[
                ("One", label_widget().text("p0")),
                ("Two", label_widget().text("p1")),
                ("Three", label_widget().text("p2")),
            ],
        )
        .unwrap();
        run_layout(
