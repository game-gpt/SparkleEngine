...
const cargoArgs = ["build", "--manifest-path", path.join(root, "Cargo.toml"), "-p", "spark-napi", "--features", "node"];
...
const outDir = path.join(root, "projects", "platforms", "native", `sparkle-engine-${plat.short}`);
...
// 清理旧名产物
for (const stale of ["spark.node", "sparkle-engine.node", `spark.${plat.triple}.node`]) {
