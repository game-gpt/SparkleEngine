...
export interface LoadOptions {
   ...
   * Wasm 请直接使用 `spark-unknown-wasm32` 的入口，不要经本函数。
   */
...
/**
 * 加载当前平台的 Spark 原生绑定。
 * 平台包是**仅含 `.node` 的二进制袋**（`main` = `spark.<triple>.node`），不是 TypeScript 包。
 */
export function loadSpark(_options: LoadOptions = {}): SparkHostBindings {
  if (_cached) return _cached;
  if (_options.platformPackage === "spark-unknown-wasm32") {
    throw new Error(
      "`loadSpark` 只加载原生 `.node`。Wasm 请使用 `spark-unknown-wasm32` 包自己的入口。",
    );
  }
